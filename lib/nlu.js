  ...
  return (err, response) => {
    if (err) {
      ...
      if (response) {
        ...
        if (response.syntax.tokens) {
          const allLemmas = response.syntax.tokens.map(token => token.lemma).filter(lemma => lemma !== undefined);
          const uniqueLemmas = Array.from(new Set(allLemmas));
          responseStats.uniqe_lemma_count = uniqueLemmas.length;
        }
